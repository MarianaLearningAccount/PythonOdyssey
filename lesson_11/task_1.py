...
# CODUL TĂU VINE MAI JOS:
def task_2(*args):
    numere_intregi = [arg for arg in args if isinstance(arg, int)]
    return numere_intregi
# CODUL TĂU VINE MAI SUS:
...
def task_5(*args, **kwargs):
    lista_intregi = sorted([arg for arg in args if isinstance(arg, int)])
    lista_stringuri = sorted([cheie for cheie, valoare in kwargs.items() if isinstance(valoare, str)])
    return lista_intregi, lista_stringuri
# CODUL TĂU VINE MAI SUS:
...
# CODUL TĂU VINE MAI JOS:
def task_8(*args):
    result = {'palindrom': [], 'non_palindrom': []}
    for arg in args:
        if str(arg) == str(arg)[::-1]:
            result['palindrom'].append(arg)
        else:
            result['non_palindrom'].append(arg)
    return result
# CODUL TĂU VINE MAI SUS:
...
def task_13(word, *args):
    ...
    for other_word in args:
        if sorted(other_word) != sorted_word:
            return False
    return True
# CODUL TĂU VINE MAI SUS:
...
def task_15(*args, sub_string):
    result = {
        'contains': [arg for arg in args if isinstance(arg, str) and sub_string in arg],
        'not_contains': [arg for arg in args if isinstance(arg, str) and sub_string not in arg]
    }
    return result
# CODUL TĂU VINE MAI SUS:
